...
const TextInput: React.FC<TextInputProps> = ({ field, onChange, onUpdate, isEditMode, index, onAutoResize, isPrintMode = false }) => {
  ...
  // Added border to navModeClasses for general visibility, but ensure it prints well.
  ...
  return (
    <div
      ...
      style={{
        ...
        zIndex: isPrintMode ? 10 : 'auto' // Ensure expanded content sits above
      }}
    >
      {isEditMode && (
        <>
      ...
      />
      <div className="absolute bottom-1 right-1 bg-blue-500 text-white text-xs rounded-sm px-1.5 py-0.5 pointer-events-none z-10 print:hidden">
        {index}
      </div>
    </div>
  );
